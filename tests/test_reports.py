...
@patch("src.reports.excel_reader")
def test_search_no_results(mock_excel):
    # Подсовываем данные, где нет "Авиабилетов"
    mock_excel.return_value = pd.DataFrame([
        {"Категория": "Продукты", "Описание": "Молоко", "Сумма": -100}
    ])
...
@patch("src.reports.excel_reader")
def test_search_empty_df(mock_excel):
    mock_excel.return_value = pd.DataFrame()
    result = search("Что угодно")
    assert json.loads(result) == []

@patch("src.reports.excel_reader")
def test_search_exception(mock_excel):
